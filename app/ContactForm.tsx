...
const formSchema = z.object({
  ...
  challenge: z.string().min(10, "Please provide some details about your project"),
});
...
export default function ContactForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xqeezzzd", {
        method: "POST",
        headers: {
          ...
          <div>
            {isSuccess ? (
              ...
              >
                ...
                <p className="text-slate-400 text-sm">I'll review your project details and get back to you shortly.</p>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <textarea {...register("challenge")} rows={4} className="w-full bg-slate-950 border border-slate-800 rounded-sm p-3 text-slate-200 focus:outline-none focus:border-cyan-500 transition-colors resize-none" placeholder="Describe your project goals, technical stack, and any specific challenges..." />
                ...
                <button type="submit" disabled={isSubmitting} className="w-full bg-slate-200 hover:bg-white text-slate-900 font-bold py-3 rounded-sm transition-all flex items-center justify-center gap-2 mt-2">
                  {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <>SEND MESSAGE <Send className="w-4 h-4" /></>}
                </button>
              </form>
            )}
          </div>
